/* Para obtener los datos sería por ejemplo:
const sudaderasDamas = data.damas.sudaderas.map((sud) => sud);*/

const data = [
    
        
            {
                genre: 'Damas',
                //name: 'Nike Sportswear oversize - Mujer',
                name:'sudadera',
                description: 'Combina tu estilo con esta amplia sudadera con capucha de tejido French terry. Las costuras sobrehiladas y elevadas, junto con las intensas combinaciones de colores, resaltan su estructura tipo patchwork para aportar un toque innovador a tus básicos para el día a día.',
                 size: 'S-M-L',
                color: 'Negro',
                stock: 1346,
                price: 74.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/a5aef494-2caa-4b56-a2b9-f886a08e2988/sportswear-sudadera-con-capucha-oversize-C1lS6b.png'
            },
            {
                genre: 'Damas',
                name:'sudadera',
                //name: 'Jordan Essentials - Sudadera de chándal',
                description: 'Luce un look básico con esta sudadera de chándal de tejido Fleece. Está confeccionada con un tejido French terry de alta densidad, cepillado para ofrecer suavidad, y cuenta con un ajuste oversize.',
                size: 'S-M-L',
                color: 'Sanddrift',
                stock: 2050,
                price: 79.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/eec5b441-96a1-49d6-a4c2-72dd5fa23b7a/jordan-essentials-sudadera-de-chandal-de-tejido-fleece-ZS5qzn.png'
            },
            {  
                genre: 'Damas',
                name:'sudadera',
                //name: 'Nike ACG Therma-FIT “Wolf Tree”',
                description: 'Vístete con la calidez premium de la parte de arriba Nike ACG Therma-FIT “Wolf Tree”. Cuenta con un ajuste holgado, amplio y suave. Los detalles discretos rinden homenaje a Smith Rock de Oregón, que sirvió de inspiración para el diseño de esta sudadera de tejido Fleece. Este producto está confeccionado al 100 % con poliéster reciclado y fibras de nylon recicladas.',
                size: 'S-M-L',
                color: 'Hazel Rush/Negro/Summit White',
                stock: 40,
                price: 134.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/e230fcfd-2237-4c6e-9028-ee370f0894e2/acg-wolf-tree-sudadera-de-media-cremallera-con-estampado-4tMccj.png'
            },
        
//----------------------
    
            {
                genre: 'Damas',
                //name: 'Nike Dri-FIT Fast',
                name:'pantalon',
                description: 'Los leggings Nike Dri-FIT Fast combinan la suavidad de uno de tus favoritos de siempre con la frescura que necesitas para correr cuando hace calor. El almacenamiento importa, por lo que hemos añadido tres bolsillos de distintos tamaños para guardar las llaves, el teléfono y la tarjeta para cuando lo necesites. Este producto se ha confeccionado con, al menos, un 75 % de fibras de poliéster reciclado.',
                size: '7-8',
                color: 'Madder Root',
                stock: 205,
                price: 60,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/871d2f33-cbf0-47be-afc2-7ade83102cb2/dri-fit-fast-leggings-de-running-de-7-8-de-talle-medio-cplSSN.png'
            },
            {
                genre: 'Damas',
                name:'pantalon',
                //name: 'Nike Dri-FIT',
                description: 'Vístete para tu carrera diaria con el mejor estilo. Estos leggings son ligeros y transpirables para mantener la frescura y la sujeción en cada pisada. Incluye muchos bolsillos para guardar tus básicos de forma segura. Este producto se ha confeccionado con, al menos, un 75 % de fibras de poliéster reciclado',
                size: '7-8',
                color: 'Sangria/Burdeos',
                stock: 300,
                price: 74.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/faad3920-de5b-4b1b-9713-4ed2c9043a11/dri-fit-leggings-de-7-8-de-talle-medio-de-running-4szWwC.png'
            },
            {
                genre: 'Damas',
                name:'pantalon',
                //name: 'Nike Yoga Dri-FIT',
                description: 'Este ligero pantalón de tejido Woven te hace sentir como en casa. Con un diseño que se inspira en los textiles y motivos de la ropa de estar en casa, el estampado por toda la prenda ofrece comodidad y un toque nostálgico',
                size: '7-12',
                color: 'Oil Green',
                stock: 32,
                price: 54.99,
                image: 'https://images.nike.com/is/image/DotCom/DM7023_386?$NIKE_PWP_GRAY$&wid=420&hei=420'
            },
        

        
            {
                genre: 'Damas',
                name:'zapatillas',
                //name: 'Nike Air Zoom Alphafly',
                description: 'Equípate para dar lo mejor de ti con las Nike Air Zoom Alphafly NEXT% Flyknit. La espuma reactiva y las dos unidades Zoom Air visibles se combinan para impulsar tu forma de correr de cara a tu próxima maratón o carrera.',
                size: '35-43',
                color: 'Hyper Violet',
                stock: 1220,
                price: 299.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/f40e2a9d-70cf-4452-bea5-a009a19c3f62/air-zoom-alphafly-next-flyknit-zapatillas-de-competicion-carretera-qD3f1d.png'
            },
            {
                genre: 'Damas',
                name:'zapatillas',
                //name: 'Nike Air Zoom Pegasus',
                description: 'Dirígete a la pista y prepárate para despegar a lomos de nuestras todoterreno con alas. Este increíble modelo regresa con un extra de elasticidad perfecto para correr sobre asfalto. Perfectas para carreras cortas o largas, te ofrecen la misma pisada elástica y sujeción amortiguada que sus predecesoras. La parte superior de malla transpirable combina la comodidad y la durabilidad que necesitas con un ajuste más amplio en la puntera.',
                size: '35-41',
                color: 'Negro/Off Noir/Flash Crimson/Hyper Violet',
                stock: 354,
                price: 1150,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/16b6acf7-9be3-4f81-9513-322382da742f/air-zoom-pegasus-38-zapatillas-de-running-carretera-zkH56D.png'
            },
            {
                genre: 'Damas',
                name:'zapatillas',
                //name: 'Nike Wildhorse 7',
                description: 'Enfréntate a las carreras de trail más duras y extremas gracias al resistente diseño de las Nike Wildhorse 7.Corre con seguridad por terrenos rocosos con goma de alta resistencia a la abrasión en la suela exterior para una tracción duradera.La parte superior aporta ventilación, durabilidad y sujeción donde más las necesitas',
                size: '35-42',
                color: 'Pure Platinum/Cool Grey/University Blue/Plata metalizado',
                stock: 220,
                price: 129.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/8f15f60f-151f-4e0f-8625-d28491814cb6/air-zoom-superrep-3-zapatillas-las-clases-de-hiit-bjJmZ8.png'
            },
        
    


    
        
            {
                genre: 'Caballeros',
                //name: 'Nike ESC',
                name:'sudadera',
                description: 'Confeccionada en Milán (Italia) con especial atención a cada detalle, esta sudadera con capucha y cremallera completa se ha diseñado para ofrecer la temperatura perfecta, la máxima ligereza y una comodidad de 360 grados. La suave combinación de algodón, cachemira y spandex queda plana al contacto con la piel para mayor comodidad.',
                size: 'S-M-L',
                color: 'Black',
                stock: 3700,
                price: 649.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/f66d6bcb-f7a8-423a-bfef-bb8acdae9e62/esc-sudadera-con-capucha-de-tejido-fleece-con-cremallera-completa-cgmBnH.png'
            },
            {
                genre: 'Caballeros',
              //name: 'Club América',
                name:'sudadera',
                description: 'La sudadera Club América ofrece la temperatura perfecta y la máxima ligereza gracias al suave tejido French terry para ofrecer comodidad en el campo durante tu rutina diaria. Los detalles premium, como el estampado "Nike Football" en varios idiomas de la cremallera, rinden homenaje a la cultura del fútbol.',
                size: 'XS-S',
                color: 'Black / White',
                stock: 1200,
                price: 64.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/eba0dc17-218d-4d58-98aa-09ccd536211c/club-america-sudadera-con-capucha-de-tejido-fleece-2cnh7t.png'
            },
            {
                genre: 'Caballeros',
              //name: 'Jordan Dri-FIT Air',
                name:'sudadera',
                description: 'La sudadera con capucha de tejido Fleece Jordan Dri-FIT Air es ideal para cualquier ocasión. Su diseño, ideal dentro y fuera de la cancha, combina la funcionalidad y el look urbano con materiales de alto rendimiento y un ajuste a medida. Este producto se ha confeccionado con, al menos, un 50 % de fibras de poliéster reciclado.',
                size: 'XS - XL',
                color: 'Gym Red',
                stock: 150,
                price: 59.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/e00fbf3d-908a-481b-8265-69a92ad0e5af/jordan-dri-fit-air-sudadera-con-capucha-de-tejido-fleece-VMbzfg.png'
            },
        

        
            {
                genre: 'Caballeros',
              //name: 'Nike Dri-FIT F.C. Libero',
                name:'pantalon',
                description: 'El pantalón F.C. Libero con capilarización del sudor ofrece el look de la equipación para antes del partido y una suavidad perfecta para el día a día. Cuenta con un ajuste holgado y ligeramente elástico para ofrecer libertad de movimiento durante todo el día',
                size: 'S - XL',
                color: 'Black',
                stock: 3025,
                price: 59.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/4387117d-10d6-4eef-b752-95f01a6e9082/dri-fit-fc-libero-pantalon-de-futbol-dVMvpL.png'
            },
            {
                genre: 'Caballeros',
              //name: 'Nike Sportswear Club',
                name:'pantalon',
                description: 'Los Jogger Nike Sportswear combinan una silueta clásica con un tejido ligero para ofrecer un look versátil y mejorado para los días más cálidos.',
                size: 'S-M-L',
                color: 'Dark Marina blue',
                stock: 1705,
                price: 49.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/0384b918-7643-44ef-bcff-a74270650ebf/sportswear-club-jogger-vbZdxz.png'
            },
            {
                genre: 'Caballeros',
              //name: 'Nike Air',
                name:'pantalon',
                description: 'El jogger Nike Air combina el estilo deportivo de los 90 con un look impecable para lucir las zapatillas. El suave tejido Fleece cepillado en la parte trasera ofrece calidez y comodidad a tu armario del día a día',
                size: 'S - XL',
                color: ' Blanco/Football Grey/Vivid Sulfur',
                stock: 1258,
                price: 74.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/0c933c07-e8e5-4c1b-b022-979c26448547/air-jogger-de-tejido-fleece-cepillado-en-la-parte-trasera-fHTgw4.png'
            },
        

        
            {
                genre: 'Caballeros',
              //name: 'Nike Metcon 7 FlyEase',
                name:'zapatillas',
                description: 'Las Nike Metcon 7 son ideales para el entrenamiento con pesas, y ofrecen un estilo más resistente y estable que en las versiones anteriores.Además, hemos añadido espuma React que mejora la comodidad para prepararte para los entrenamientos de alta intensidad',
                size: '39-43',
                color: 'Total Orange/Dark Smoke',
                stock: 1541,
                price: 129.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/fb149f6b-c850-4708-ac13-1c2cb4df89d5/metcon-7-flyease-zapatillas-de-entrenamiento-RKGv1m.png'
            },
            {
                genre: 'Caballeros',
              //name: 'Nike Air Max Plus',
                name:'zapatillas',
                description: 'Deja que tu estilo se eleve con las Nike Air Max Plus, una experiencia Air actualizada que ofrece una estabilidad premium y una amortiguación increíble. Al igual que las originales, este modelo cuenta con las líneas de diseño onduladas, los detalles de TPU y la malla ligera en la parte superior para rendir homenaje a su estilo desafiante.',
                size: '38-49',
                color: 'Black',
                stock: 1265,
                price: 169.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/erzfadfpnzgkxt0gn2ya/air-max-plus-zapatillas-07mFHd.png'
            },
            {
                genre: 'Caballeros',
              //name: 'Nike Air Max Terrascape Plus',
                name:'zapatillas',
                description: 'Párate a pensar en ese momento de pausa que se produce cuando ves algo nuevo,algo emocionante.Eso es lo que representan las Nike Air Max Terra',
                size: '36-49',
                color: 'Blanco perla/Dark Beetroot/Canvas/Hot Curry',
                stock: 155,
                price: 197.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/7a1eb698-306b-4e4d-a803-7b7e698f2abc/air-max-terrascape-plus-zapatillas-Nc2zZ7.png'
            },
        
    


    {
    
      
                genre: 'Niños',
              //name: 'Nike Sportswear Club - Niño/a',
                name:'sudadera',
                description: 'Un diseño ideal para combinarlo con otras prendas o llevarlo solo durante los entrenamientos de los días más fríos.La sudadera con capucha Nike Sportswear está confeccionada con un cálido tejido Fleece cepillado en el interior para ofrecer una suavidad extraordinaria',
                size: 'XS - M',
                color: 'rojo',
                stock: 1200,
                price: 39.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/kzrfjedxnbsju4wia0ba/sportswear-club-sudadera-con-capucha-nino-a-qgc3Mt.png'
            },
            {
                genre: 'Niños',
              //name: 'Nike Sportswear French terry - Niño',
                name:'sudadera',
                description: 'La sudadera Nike Sportswear es suave y cómoda, por lo que es ideal para disfrutar de una capa de calidez adicional',
                size: 'XS - M',
                color: ' Carbon Heather/Blanco',
                stock: 120,
                price: 34.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/061909dc-9910-4bd9-aec4-b9a226543efa/sportswear-sudadera-de-tejido-french-terry-nino-R0VmDv.png'
            },
            {
                genre: 'Niños',
              //name: 'Nike Sportswear Club Fleece',
                name:'sudadera',
                description: 'La sudadera con capucha Nike Sportswear es suave, cómoda y cálida, como cualquier sudadera favorita. El bolsillo práctico, el logotipo exclusivo y el tejido Fleece aportan un toque clásico que pueden llevar durante todo el día y siempre que quieran.',
                size: 'S-XL',
                color: 'Marina/Negro',
                stock: 1235,
                price: 54.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/b522bfc6-47b7-41ad-8194-8da224101fd3/sportswear-club-fleece-sudadera-con-capucha-nino-LKSdbG.png'
            },
            {
                genre: 'Niños',
              //name: 'Nike Sportswear - Niña',
                name:'sudadera',
                description: 'La camiseta Nike Sportswear está confeccionada para ofrecer una mayor versatilidad. El suave tejido Knit de estilo Henley y el colorido estampado floral permiten que la combines con leggings, vaqueros o tus mejores pantalones. Esta camiseta muy versátil y divertida aporta un ajuste cómodo para añadir a cualquier armario.',
                size: 'S-XL',
                color: 'University Blue/Blanco',
                stock: 123,
                price: 48.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/a29c2c6f-75c6-48d6-a0b4-d1561bced402/sportswear-camiseta-de-manga-larga-nina-nPkg0l.png'
            },
        

       
            {
                genre: 'Niños',
              //name: 'Nike Sportswear - Jogger de tejido Fleece - Niño',
                name:'pantalón',
                description: 'Confeccionado con un cálido tejido Fleece cepillado, este jogger para el día a día ofrece la comodidad suave y agradable que tanto gusta a los más pequeños.',
                size: 'XS - XL',
                color: 'Team Red/Dark Smoke',
                stock: 1325,
                price: 44.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/0f68078a-8c07-4b43-b9ac-3831d5e40968/sportswear-jogger-de-tejido-fleece-nino-W5ZNc1.png'
            },
            {
                genre: 'Niños',
              //name: 'Nike Sportswear Kids Pack - Unisex',
                name:'pantalón',
                description: 'Hay que seguir jugando y compitiendo en carreras. Sabemos que lo mejor es jugar sin pensar en nada, aunque a veces no se puede. Así hemos confeccionado nuestro pantalón. Los bolsillos de fácil acceso son perfectos para que nada salga volando al jugar, saltar o liderar el grupo.',
                size: 'XS - M',
                color: 'Mint Foam',
                stock: 159,
                price: 64.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/b3a9f2e7-46f1-4714-a883-5e1ca9ad2af4/sportswear-pack-pantalon-de-tejido-woven-nino-a-ntp6kf.png'
            },
            {
                genre: 'Niños',
              //name: 'Nike Sportswear Fleece - Niña',
                name:'pantalón',
                description: 'El pantalón Nike Sportswear está confeccionado con un tejido Fleece supersuave y cómodo. Además, cuentan con un ajuste entallado para ofrecer la máxima comodidad sin añadir un look abultado',
                size: 'XS - L',
                color: 'Carbon Heather/Negro/Rush Pink',
                stock: 447,
                price: 37.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/bd7aed01-2f1c-4419-8b06-7e9a0192fb16/sportswear-pantalon-de-tejido-fleece-nina-2TsV9G.png'
            },
        

       
            {
                genre: 'Niños',
              //name: 'Nike Dynamo Go',
                name:'zapatillas',
                description: 'Las Nike Dynamo Go tienen un diseño sencillo, ceñido e ideal para jugar, por lo que serán el próximo modelo favorito de los más pequeños. Lo mejor es que son fáciles de poner gracias al talón plegable. Si los más pequeños no pueden hacerlo solos, enséñales. Además, ofrecen suavidad en los mejores lugares para que no pare la diversión.',
                size: '27-35',
                color: 'rojo',
                stock: 125,
                price: 64.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/cc622c2f-b34c-45a0-975f-3e1135ea7f79/dynamo-go-zapatillas-faciles-de-poner-y-quitar-nino-a-pequeno-a-qmfPzS.png'
            },
            {
                genre: 'Niños',
              //name: 'Nike Flex Advance',
                name:'zapatillas',
                description: 'Las Nike Flex Advance no podrían ser más cómodas.La tecnología Nike FlyEase presenta una abertura extraancha para poder ponérselas con facilidad a los más pequeños (de verdad, es pan comido).Entrecruza las correas (para que los más pequeños se vayan acostumbrando a los cordones de verdad) y ¡listo!Son extremadamente flexibles, cómodas y duraderas, por lo que resultan perfectas para que los más pequeños caminen, corran y jueguen.',
                size: '24-35',
                color: 'Medium Ash/Canyon Rust/Kumquat/Negro',
                stock: 159,
                price: 59.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/90f5df37-fd2e-4cf5-b279-5f4077354b13/flex-advance-zapatillas-nino-a-pequeno-a-L44DwQ.png'
            },
            {
                genre: 'Niños',
              //name: 'Nike Huarache Run',
                name:'zapatillas',
                description: 'Las Nike Huarache Run son inconfundibles.La famosa correa de piel Huarache destaca en este modelo, sencillo de poner para los más pequeños.El diseño de botín con ajuste ceñido mantiene la seguridad y la sujeción de los más pequeños al correr, escalar y jugar.',
                size: '24-37',
                color: 'Marina/Armory Navy/Blanco/Negro',
                stock: 1589,
                price: 84.99,
                image: 'https://static.nike.com/a/images/t_PDP_864_v1/f_auto,b_rgb:f5f5f5/f63ed6df-a91c-4526-83f8-38befda84fb6/huarache-run-zapatillas-nino-a-pequeno-a-zsL2zh.png'
            },
       
    
];

 module.exports = data;